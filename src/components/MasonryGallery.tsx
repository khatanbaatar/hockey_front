...
const MasonryGallery = ({ media, onImageClick }: MasonryGalleryProps) => {
  ...
  const currentItems = media.slice(startIndex, endIndex);

  // Responsive column calculation (max 4 columns)
    ...
    const updateColumns = () => {
      if (typeof window !== 'undefined') {
        ...
        if (width < 768) {
          ...
          setColumns(maxColumns); // Maximum 4 columns
        }
      }
    };
  ...
  const getItemHeight = (index: number) => {
    const heights = [180, 220, 260, 300, 240, 280, 320, 200, 340, 250, 380, 210, 360, 290, 310, 230, 350, 270, 330, 190];
  ...
  return (
    <div className="masonry-gallery-container">
                  ...
                  {item.type === 'image' ? (
                    <div className="relative">
                      <Image
                        ...
                        onClick={() => {
                          if (onImageClick) {
                            const imageIndex = currentItems.filter(m => m.type === 'image').findIndex(m => m.url === item.url);
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-300">
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                          <button
                            ...
                            className="size-12 inline-flex items-center justify-center bg-white/90 hover:bg-white text-gray-700 hover:text-indigo-600 rounded-full shadow-lg transition-all duration-300 transform hover:scale-110"
      ...
      <div className="mt-6 text-center text-gray-600">
        <p className="text-sm">
          Хуудас {currentPage} / {totalPages} • Нийт {media.length} зүйл
        </p>
      </div>
    </div>
  );
